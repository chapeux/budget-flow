...
import { Sparkles, ArrowRight, X, EyeOff, BrainCircuit, LineChart, Wallet, Home, Check, Plus, AlertCircle, MessageSquareQuote } from 'lucide-react';
        ...
        <div className="p-6">
            {activeTab === 'GENERATE' ? (
                <div className="space-y-6">
                    <div className="bg-indigo-50/30 dark:bg-indigo-900/10 p-5 rounded-xl border border-indigo-100 dark:border-indigo-800">
                        <div className="flex flex-col md:flex-row gap-6 items-start md:items-center justify-between mb-6">
                            ...
                            <div className="flex items-center gap-6">
                                ...
                                <Button 
                                    ...
                                    className="bg-indigo-600 text-white hover:bg-indigo-700 shadow-md font-bold text-xs"
                                >
                                    Sugerir Estrutura
                                </Button>
                            </div>
                        ...
                        <div className="space-y-2 pt-2 border-t border-indigo-100 dark:border-indigo-800/50">
                            <label className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest flex items-center gap-2">
                                ...
                                Despesas Obrigatórias (Restrições)
                            </label>
                            <textarea 
                                value={mandatoryExpenses}
                                onChange={(e) => setMandatoryExpenses(e.target.value)}
                                placeholder='Ex: "Pago R$910 de faculdade e no mínimo R$800 reais de gasolina"'
                                className="w-full min-h-[60px] p-3 rounded-lg border border-indigo-100 dark:border-indigo-800 bg-white/50 dark:bg-slate-900/50 text-sm outline-none focus:ring-2 focus:ring-indigo-500 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-600 italic"
                            />
                            <p className="text-[10px] text-slate-400">A IA irá priorizar esses valores na montagem do seu plano.</p>
                        </div>
                    </div>
                                ...
                                <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-700">
                                    <div className="flex items-center justify-between mb-4">
                                        <h4 className="text-sm font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider flex items-center gap-2">
                                            <Check className="w-4 h-4" /> Itens Propostos
                                        </h4>
                                            ...
                                            <div key={i} className={`flex justify-between items-center p-2 rounded-lg border ${item.isInvestment ? 'bg-blue-50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-800' : 'bg-slate-50 dark:bg-slate-900 border-slate-100 dark:border-slate-800'}`}>
                                                <div className="min-w-0">
                                                    <p className="text-xs font-bold truncate dark:text-white">
                                                        {item.name}
                                                        {item.isInvestment && <span className="ml-1.5 text-[8px] bg-blue-600 text-white px-1 rounded">INV</span>}
                                                    </p>
                                                    <p className="text-[10px] text-slate-400">
                                                        {item.category} 
                                                        {item.isInvestment && item.annualRate && ` • ${item.annualRate}% a.a`}
                                                    </p>
                                                </div>
                                                <p className={`text-xs font-black ml-2 ${item.isInvestment ? 'text-blue-600 dark:text-blue-400' : 'text-indigo-600 dark:text-indigo-400'}`}>
                                    ...
                                    >
                                        ...
                                        Aplicar ao meu Plano Financeiro
                                    </Button>
                                </div>
                            )}
                        </div>
                    ...
                    {totalIncome <= 0 && (
                        <div className="flex items-center gap-3 p-4 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 rounded-lg border border-amber-100 dark:border-amber-800">
                            <AlertCircle className="w-5 h-5 shrink-0" />
                            <p className="text-xs font-medium">Por favor, cadastre sua <strong>renda</strong> primeiro para que a IA possa sugerir um orçamento adequado.</p>
                        </div>
                    )}
                </div>
            ) : (
                ...
                <div className="flex flex-col items-center justify-center py-6 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-dashed border-slate-200 dark:border-slate-700">
                    ...
                    <div className="mt-2 bg-indigo-50/50 dark:bg-indigo-900/20 rounded-xl p-6 border border-indigo-100 dark:border-indigo-900/50 animate-in fade-in slide-in-from-top-2 shadow-sm relative overflow-hidden">
                        ...
                        <div className="prose prose-indigo prose-sm max-w-none text-slate-700 dark:text-slate-300 dark:prose-headings:text-indigo-300 dark:prose-strong:text-indigo-300">
                            <ReactMarkdown>{activeTab === 'BUDGET' ? budgetAnalysis! : investmentAnalysis!}</ReactMarkdown>
                        </div>
